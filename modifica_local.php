...
$query="update usuario set persona_contacto='$nombre', telefono='$telefono',descripcion_local='$descripcion', cod_ciudad=$ciudad where codigo_usuario=$codigo;";

$db->executer($query);

echo "<meta http-equiv='Refresh' content='0;url=perfil_local.php?id=".$_SESSION['codigo']."'>";
}

else{
$codigo= $_GET['id'];
$query="select u.persona_contacto,u.telefono,u.descripcion_local ,c.nombre  as 'cnombre'  from usuario u inner join ciudad c on u.cod_ciudad=c.codigo where u.codigo_usuario=$codigo;";
	$db->executer($query);
...
echo "<label><span>nombre:</span><input type='text' name='nombre' value='".$local['persona_contacto']."'required/></label>";
	echo "<label><span>telefono:</span><input type='text' name='telefono' value='".$local['telefono']."' required/></label>";
...
echo "<label><span>Informacion </span><textarea rows='4' cols='50' name='descripcion'  >".$local['descripcion_local']."</textarea required></label>";
}
