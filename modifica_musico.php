...
$query="update usuario set nombre_grupo='$nombre', telefono='$telefono',edad=$edad,descripcion_musico='$descripcion', cod_ciudad=$ciudad where codigo_usuario=$codigo;";

$db->executer($query);

echo "<meta http-equiv='Refresh' content='0;url=musico.php?id=".$_SESSION['codigo']."'>";
}

else{
$codigo= $_GET['id'];
$query="select u.nombre_grupo,u.telefono,u.descripcion_musico,edad ,c.nombre  as 'cnombre'  from usuario u inner join ciudad c on u.cod_ciudad=c.codigo where u.codigo_usuario=$codigo;";
	$db->executer($query);
...
echo "<label><span>nombre:</span><input type='text' name='nombre' value='".$musico['nombre_grupo']."'required/></label>";
	echo "<label><span>telefono:</span><input type='text' name='telefono' value='".$musico['telefono']."' required/></label>";
	echo "<label><span>telefono:</span><input type='text' name='edad' value='".$musico['edad']."' required/></label>";
...
foreach($result as $ciudad){
	if($ciudad['nombre']==$musico['cnombre']){
...
echo "<label><span>Informacion </span><textarea rows='4' cols='50' name='descripcion'  >".$musico['descripcion_musico']."</textarea required></label>";
}
